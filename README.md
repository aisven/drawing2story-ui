...
## Notes to get a working Installation

This section contains important notes to get a working installation

### All OSes

The project is using the Natural Language Toolkit (NLTK).
During runtime, the toolkit needs some of its own data files in order to work,
for example to correctly tokenize English text.
Therefore, after installing the dependencies using poetry (see below),
please execute the following commands in a python interpreter:
...
This will open the NLTK downloader which is a UI where you can choose
which NLTK data files should be downloaded.
A typical setup would be to just download everything, i.e. to keep everything selected,
and to have the downloader download the files to the directory `nltk_data` within your home directory.
After the download has finished, you can just close the downloader.
See: https://www.nltk.org/data.html

### macOS

Under macOS please install the following to make the installation of the library `PyAudio` work:

`brew install portaudio`

See: https://formulae.brew.sh/formula/portaudio
